              ...
              {resetEmailSent && isForgotPasswordMode ? (
                ...
                <>
                  <div>
                    ...
                    <input type="email" required value={authEmail} onChange={(e) => setAuthEmail(e.target.value)} className="w-full bg-[#f4f4f4] border border-neutral-300 p-3 text-sm font-bold outline-none focus:border-black transition-colors" placeholder="vas@email.cz" />
                  </div>

                  {!isForgotPasswordMode && (
                    <div>
                      <label className="block text-[10px] font-bold uppercase tracking-wider text-neutral-500 mb-1">Heslo</label>
                      <input type="password" required value={authPassword} onChange={(e) => setAuthPassword(e.target.value)} className="w-full bg-[#f4f4f4] border border-neutral-300 p-3 text-sm font-bold outline-none focus:border-black transition-colors" placeholder="Minimálně 6 znaků" minLength={6} />
                      {isLoginMode && (
                        <div className="text-right mt-2">
                          <button type="button" onClick={() => setIsForgotPasswordMode(true)} className="text-[10px] text-neutral-500 hover:text-black uppercase font-bold transition-colors cursor-pointer underline">Zapomněli jste heslo?</button>
                        </div>
                      )}
                    </div>
                  )}
