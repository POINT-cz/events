...
export default function GlobalModals({
  ...
  return (
    <>
      {/* COOKIE BANNER */}
      {showCookieBanner && (
        <div className="fixed bottom-0 left-0 right-0 bg-[#f4f4f4] text-black p-4 sm:p-6 z-[300] flex flex-col sm:flex-row justify-between items-center gap-4 border-t border-neutral-300 pointer-events-auto font-mono">
          <div className="text-xs sm:text-sm max-w-4xl text-center sm:text-left">
          ...
          <button onClick={handleAcceptCookies} className="shrink-0 bg-black text-white px-6 py-2.5 font-bold uppercase tracking-wider hover:bg-[#E4664F] transition-colors text-xs w-full sm:w-auto border border-neutral-300 cursor-pointer">Rozumím</button>
        </div>
      )}

      {/* GDPR MODAL */}
      {showGdprModal && (
        <div className="fixed inset-0 z-[250] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 pointer-events-auto font-mono" onClick={() => setShowGdprModal(false)}>
          <div className="bg-[#f4f4f4] border border-neutral-300 p-6 sm:p-8 rounded-none w-full max-w-2xl overflow-y-auto max-h-[85vh] animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
            ...
            <div className="text-xs sm:text-sm text-black/80 space-y-4">
            ...
            <div className="mt-8 pt-4 border-t border-neutral-300 flex justify-end">
              <button onClick={() => setShowGdprModal(false)} className="px-6 py-3 bg-black text-white rounded-none text-xs font-bold uppercase tracking-wider hover:bg-[#E4664F] transition-colors cursor-pointer border border-neutral-300">Rozumím a zavřít</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* VOP A STORNO MODAL */}
      {showVopModal && (
        <div className="fixed inset-0 z-[250] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 pointer-events-auto font-mono" onClick={() => setShowVopModal(false)}>
          <div className="bg-[#f4f4f4] border border-neutral-300 p-6 sm:p-8 rounded-none w-full max-w-2xl overflow-y-auto max-h-[85vh] animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
            ...
            <div className="mt-8 pt-4 border-t border-neutral-300 flex justify-end">
              <button onClick={() => setShowVopModal(false)} className="px-6 py-3 bg-black text-white rounded-none text-xs font-bold uppercase tracking-wider hover:bg-[#E4664F] transition-colors cursor-pointer border border-neutral-300">Rozumím a zavřít</button>
            </div>
          </div>
        </div>
      )}
      ...
      {selectedTicket && (
        <div className="fixed inset-0 z-[150] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 pointer-events-auto font-mono" onClick={() => setSelectedTicket(null)}>
            <div className="bg-[#f4f4f4] border border-neutral-300 p-6 sm:p-8 rounded-none max-w-sm w-full text-center animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
                ...
                <p className="text-sm font-medium text-black/60 mb-6 sm:mb-8">{formatDateCzech(selectedTicket.date)}</p>
                
                <div className="bg-white p-4 border border-neutral-300 mb-6">
                  {ticketQr ? <img src={ticketQr} alt="QR Ticket" className="mx-auto w-48 h-48 sm:w-64 sm:h-64 mix-blend-multiply" /> : <div className="w-48 h-48 sm:w-64 sm:h-64 bg-neutral-200 animate-pulse mx-auto border border-neutral-300" />}
                </div>
                
                <p className="text-[10px] text-black break-all bg-white p-2 border border-neutral-300 mb-2">ID: {selectedTicket.id}</p>
                <button onClick={() => setSelectedTicket(null)} className="mt-6 w-full py-3 bg-black text-white text-xs font-bold uppercase tracking-widest transition-colors hover:bg-[#E4664F] cursor-pointer border border-neutral-300">Zavřít</button>
            </div>
        </div>
      )}
      ...
      {/* ZPRACOVÁNÍ NÁKUPU */}
      {isSubmitting && (
        <div className="fixed inset-0 z-[200] bg-black/40 backdrop-blur-sm flex flex-col items-center justify-center p-4 font-mono">
           <div className="bg-[#f4f4f4] border border-neutral-300 p-10 rounded-none flex flex-col items-center max-w-sm w-full animate-in zoom-in-95">
              <div className="grow-dot mb-8"></div>
              <h3 className="font-bold text-lg text-black uppercase mb-1">Zpracovávám požadavek</h3>
              <p className="text-xs font-medium text-black/60 text-center">Ukládám do databáze...</p>
           </div>
        </div>
      )}
    </>
