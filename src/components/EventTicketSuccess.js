...
export default function EventTicketSuccess({ lastCreatedRes, qrCodeUrl, setBookingStep, setSelectedEvent, setLastCreatedRes }) {
  return (
    <div className="max-w-md mx-auto w-full bg-white border border-neutral-300 p-6 sm:p-10 text-center space-y-6 font-mono animate-in fade-in">
      ...
      <div className="p-6 bg-[#f4f4f4] border border-neutral-300 w-full space-y-3">
        {qrCodeUrl ? (
          <img src={qrCodeUrl} alt="QR Platba" className="mx-auto w-40 h-40 border border-neutral-300 bg-white p-2" />
        ) : (
          <div className="w-40 h-40 bg-neutral-200 animate-pulse mx-auto border border-neutral-300" />
        )}
        <div className="mt-4">
