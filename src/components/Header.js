export default function Header({
  user, displayName, view, setView, showUserMenu, setShowUserMenu, handleLogout,
  setIsLoginMode, setIsForgotPasswordMode, setResetEmailSent, setShowAuthModal, 
  setGdprConsent, setBookingStep, setSelectedEvent
}) {
  return (
    <header className="bg-white border-b border-gray-200 sticky top-0 z-40 shadow-sm w-full">
      <div className="px-4 sm:px-8 py-4 flex justify-between items-center">
        <div className="flex items-center shrink-0 cursor-pointer pointer-events-auto" onClick={() => { setView('events_portal'); setBookingStep(1); setSelectedEvent(null); }}>
        ...
        <div className="flex items-center space-x-3 sm:space-x-6 text-sm font-medium justify-end min-w-0">
          {user ? (
            <div className="relative pointer-events-auto">
              <button onClick={() => setShowUserMenu(!showUserMenu)} className={`flex items-center gap-2 text-sm font-bold transition-colors ${(view === 'client_dashboard' || view === 'client_profile' || view === 'client_favorites') ? 'text-red-600' : 'text-slate-800 hover:text-red-600'} max-w-[140px] sm:max-w-xs`}>
              ...
              {showUserMenu && (
                <div className="absolute right-0 top-full mt-3 w-56 bg-white rounded-xl shadow-xl border border-gray-100 overflow-hidden z-50 animate-in fade-in">
                  ...
                  <button onClick={() => { setView('client_dashboard'); setShowUserMenu(false); }} className="w-full text-left px-5 py-3 text-sm font-medium text-slate-600 hover:bg-slate-50 transition-colors">Moje vstupenky</button>
      ...
      {/* Sekundární menu upravené čistě pro eventový portál */}
      <div className="px-4 sm:px-8 pb-4 flex justify-start sm:justify-center overflow-x-auto hide-scrollbar pointer-events-auto w-full">
        <div className="flex space-x-1.5 bg-slate-100 p-1.5 rounded-xl shadow-inner whitespace-nowrap shrink-0">
          <button onClick={() => { setView('events_portal'); setBookingStep(1); setSelectedEvent(null); }} className={`px-4 sm:px-5 py-2 sm:py-2.5 text-xs sm:text-sm font-bold rounded-lg transition-all ${view === 'events_portal' ? 'bg-white text-red-600 shadow-sm' : 'text-slate-500 hover:text-slate-800'}`}>Katalog Akcí</button>
          <div className="w-px bg-slate-200 my-2 mx-1"></div>
          <a href="https://rezervace.pointspace.cz" className="px-4 sm:px-5 py-2 sm:py-2.5 text-xs sm:text-sm font-bold rounded-lg transition-all text-slate-500 hover:text-slate-800 flex items-center gap-2">
            Zpět na rezervace prostor <span>➔</span>
          </a>
        </div>
      </div>
    </header>
