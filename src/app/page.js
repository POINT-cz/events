...
export default function EventsHome() {
  // Výchozí obrazovka jsou rovnou akce
  ...
  const [formData, setFormData] = useState({ firstName: '', lastName: '', email: '', phone: '', company: '', ico: '', dic: '', street: '', city: '', psc: '', paymentType: 'qr_code' });

  const formatDateCzech = (isoDate) => { if (!isoDate) return ''; const parts = isoDate.split('-'); if (parts.length === 3) return `${parseInt(parts[2], 10)}. ${parseInt(parts[1], 10)}. ${parts[0]}`; return isoDate; };

  // ROUTER: Pokud chce do Studia nebo Adminu, pošleme ho na hlavní doménu
  const handleSetView = (newView) => { 
    if (newView === 'client' || newView === 'admin') {
      window.location.href = 'https://rezervace.pointspace.cz';
    } else {
      setView(newView); 
    }
  };
  ...
  useEffect(() => {
    function handleClickOutside(event) { if (userMenuRef.current && !userMenuRef.current.contains(event.target)) setShowUserMenu(false); }
    document.addEventListener("mousedown", handleClickOutside);
    return () => document.removeEventListener("mousedown", handleClickOutside);
  }, [userMenuRef]);

  useEffect(() => {
    const handleMouseMove = (e) => { if (cursorRef.current) { cursorRef.current.style.left = `${e.clientX}px`; cursorRef.current.style.top = `${e.clientY}px`; } };
  ...
  const handleAcceptCookies = () => { localStorage.setItem('point_cookie_consent', 'true'); setShowCookieBanner(false); };

  useEffect(() => {
    const { data: authListener } = supabase.auth.onAuthStateChange((event, session) => {
      if (event === 'PASSWORD_RECOVERY') { setShowRecoveryModal(true); setShowAuthModal(false); }
    ...
    async function loadInitialData() {
      ...
      const { data: bookingData } = await supabase.from('reservations').select(`*`);
  ...
  useEffect(() => {
    async function loadUserData() {
      if (user) {
        ...
        setClientData(null); setFavoriteEvents([]); setFormData({ firstName: '', lastName: '', email: '', phone: '', company: '', ico: '', dic: '', street: '', city: '', psc: '', paymentType: 'qr_code' });
      }
    }
    loadUserData();
  }, [user]);
  ...
  const toggleFavorite = async (e, eventId) => {
    e.stopPropagation();
    if (!user) { alert('Pro přidání do oblíbených se prosím přihlaste.'); setIsLoginMode(true); setShowAuthModal(true); return; }
    let newFavs = [...favoriteEvents];
    if (newFavs.includes(eventId)) newFavs = newFavs.filter(id => id !== eventId); else newFavs.push(eventId);
    setFavoriteEvents(newFavs);
    if (clientData) await supabase.from('customers').update({ favorite_events: newFavs }).eq('id', clientData.id);
  };
  ...
  const handleAuthSubmit = async (e) => {
    ...
    if (isForgotPasswordMode) {
      ...
      if (error) alert(error.message); else setResetEmailSent(true);
      setAuthLoading(false); return;
    }
    if (!isLoginMode && !gdprConsent) { alert("Prosím odsouhlaste podmínky pro vytvoření účtu."); setAuthLoading(false); return; }

    if (isLoginMode) {
      const { data, error } = await supabase.auth.signInWithPassword({ email: authEmail, password: authPassword });
      if (error) alert('Nesprávný e-mail nebo heslo.'); else { setUser(data.user); setShowAuthModal(false); setAuthEmail(''); setAuthPassword(''); }
    } else {
      const { data, error } = await supabase.auth.signUp({ email: authEmail, password: authPassword });
      if (error) alert(error.message); else { setUser(data.user); alert('Účet vytvořen.'); setShowAuthModal(false); setAuthEmail(''); setAuthPassword(''); }
    }
    setAuthLoading(false);
  };

  const handleLogout = async () => { await supabase.auth.signOut(); setUser(null); setShowUserMenu(false); setView('events_portal'); };

  const handleRecoverySubmit = async (e) => {
    e.preventDefault(); setAuthLoading(true);
    const { error } = await supabase.auth.updateUser({ password: newRecoveryPassword });
    if (error) alert('Nepodařilo se změnit heslo: ' + error.message); else { alert('Heslo bylo úspěšně změněno.'); setShowRecoveryModal(false); setNewRecoveryPassword(''); }
    setAuthLoading(false);
  };
  ...
  const handleProfilePasswordChange = async (e) => {
    ...
    try {
      ...
      if (updateError) alert('Chyba při změně hesla: ' + updateError.message); else { alert('Heslo bylo úspěšně změněno.'); setOldPassword(''); setNewProfilePassword(''); }
    } catch (err) { setSavingProfile(false); alert('Chyba: ' + err.message); }
  };

  const resetClientView = () => {
    setSelectedEvent(null); setIsDescExpanded(false); setBookingStep(1); setGdprConsent(false); setHoneypot('');
    if (!user) setFormData({ firstName: '', lastName: '', email: '', phone: '', company: '', ico: '', dic: '', street: '', city: '', psc: '', paymentType: 'qr_code' });
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };

  const calculateIban = (accountStr, bankCode) => {
    const parts = accountStr.split('-'); let prefix = parts.length > 1 ? parts[0] : ''; let base = parts.length > 1 ? parts[1] : parts[0];
    ...
    const digitString = `${cleanBank}${prefix}${base}123500`; const remainder = BigInt(digitString) % 97n;
  ...
  const handleProfileSave = async (e) => {
    e.preventDefault(); if (!user) return; setSavingProfile(true);
    const { data, error } = await supabase.from('customers').upsert({ auth_id: user.id, email: user.email, first_name: profileForm.firstName, last_name: profileForm.lastName, phone: profileForm.phone, company_name: profileForm.company, ico: profileForm.ico, dic: profileForm.dic, billing_address: profileForm.billingAddress }, { onConflict: 'email' }).select().single();
    setSavingProfile(false);
    if (error) alert('Chyba při ukládání: ' + error.message); else { alert('Vaše údaje byly úspěšně uloženy.'); setClientData(data); }
  };

  const loadFromAres = async () => {
    if (!formData.ico && view !== 'client_profile') return alert('Zadejte prosím IČO firmy.');
    setAresLoading(true); const targetIco = view === 'client_profile' ? profileForm.ico : formData.ico;
    try {
      const response = await fetch(`https://ares.gov.cz/ekonomicke-subjekty-v-be/rest/ekonomicke-subjekty/${targetIco.replace(/\s/g, '')}`);
      if (!response.ok) throw new Error('Nenalezeno.');
      const data = await response.json(); const adresa = data.sidlo || {}; const ulice = adresa.nazevUlice || adresa.nazevObce || ''; const cp = adresa.cisloPopisne || ''; const co = adresa.cisloOrientacni ? `/${adresa.cisloOrientacni}` : '';
    ...
    } catch (err) { alert('Chyba při komunikaci s ARES.'); } finally { setAresLoading(false); }
  };

  const handleClientSubmit = async (e) => {
    e.preventDefault(); if (honeypot) return; if (!gdprConsent) return alert("Je nutné souhlasit s podmínkami.");
    setIsSubmitting(true);
    
    const { data: customerData, error: custError } = await supabase.from('customers').upsert({
        first_name: formData.firstName, last_name: formData.lastName, email: formData.email, phone: formData.phone,
        company_name: formData.company, ico: formData.ico, dic: formData.dic, billing_address: `${formData.street}, ${formData.psc} ${formData.city}`.trim(), auth_id: user ? user.id : null 
    ...
    let insertData = { customer_id: customerData.id, status: 'pending_payment', payment_type: formData.paymentType, variable_symbol: vs, total_price: selectedEvent.price, event_id: selectedEvent.id, date: selectedEvent.date, notes: `Vstupenka na: ${selectedEvent.title}`, start_hour: 0, end_hour: 0, db_end_hour: 0 };
    ...
    await generateQrPayment(selectedEvent.price, vs);

    try {
      const emailTime = selectedEvent.time;
      const iban = calculateIban("1234567890", "3030"); 
      const spaydString = `SPD*1.0*ACC:${iban}*AM:${selectedEvent.price}.00*CC:CZK*X-VS:${vs}*MSG:POINT`;
      ...
      await supabase.functions.invoke('send-email', { body: { type: 'potvrzeni', to: formData.email, jmeno: formData.firstName, sluzba: selectedEvent.title, datum: formatDateCzech(insertData.date), cas: emailTime, cena: selectedEvent.price, qrPaymentUrl: qrPaymentUrl, qrTicketUrl: qrTicketUrl }});
    } catch (e) {}

    setResourcesReservations([...reservations, newBooking]); setLastCreatedRes({ ...newBooking, email: formData.email }); setIsSubmitting(false); setBookingStep(3);
  };

  // KOMPLETNÍ SCHOVÁNÍ SYSTÉMOVÉ MYŠI (VŠUDE!)
  ...
  if (loading) return (<div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center"><style dangerouslySetInnerHTML={{__html: globalAnimationCss}} /><div className="grow-dot mb-8"></div><div className="text-sm font-bold text-slate-800 animate-pulse">Načítám Point Systém...</div></div>);
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans flex flex-col relative overflow-x-hidden selection:bg-red-100">
      <style dangerouslySetInnerHTML={{__html: globalAnimationCss}} />
      <div ref={cursorRef} className="fixed w-3 h-3 bg-red-500 rounded-full pointer-events-none z-[999999] hidden md:block" style={{ transform: 'translate(-50%, -50%)', left: '-100px', top: '-100px' }} />

      <Header 
        user={user} displayName={displayName} isAdmin={isAdmin} 
        showUserMenu={showUserMenu} setShowUserMenu={setShowUserMenu} userMenuRef={userMenuRef} 
        view={view} setView={handleSetView} section={''} setSection={()=>{}} 
        resetClientView={resetClientView} setBookingStep={setBookingStep} 
        handleLogout={handleLogout} setIsLoginMode={setIsLoginMode} 
        setIsForgotPasswordMode={setIsForgotPasswordMode} setResetEmailSent={setResetEmailSent}
        setShowAuthModal={setShowAuthModal} setGdprConsent={setGdprConsent} 
      />
      ...
      <main className={`flex-1 ${view === 'events_portal' && selectedEvent ? 'p-0 w-full' : 'p-4 sm:p-8 max-w-6xl w-full mx-auto relative z-10'} flex flex-col mb-12`}>
        ...
        {/* ODKAZ ZPĚT NA HLAVNÍ WEB S REZERVACEMI */}
        {!selectedEvent && view === 'events_portal' && (
          <div className="bg-slate-100 border border-slate-200 text-slate-700 rounded-xl p-3 sm:p-4 text-center text-xs sm:text-sm font-medium mb-6">
             Potřebujete si zarezervovat Studio, Podcast nebo Coworking? <button onClick={() => window.location.href = 'https://rezervace.pointspace.cz'} className="text-red-600 hover:text-red-800 underline font-bold ml-1 pointer-events-auto">Přejděte na rezervace</button>
          </div>
        )}
        ...
        {view === 'client_favorites' && (
          <div className="max-w-6xl mx-auto w-full animate-in fade-in space-y-8 pt-4 pb-12">
            ...
            <div className="w-full">
              {dbEvents.filter(e => favoriteEvents.includes(e.id)).length === 0 ? (
                  <div className="text-center p-8 sm:p-12 bg-white rounded-none border border-gray-100 text-slate-500">Zatím nemáte žádné oblíbené akce.</div>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                  {dbEvents.filter(e => favoriteEvents.includes(e.id)).map((event) => {
                    const isFav = true; 
                    return (
                      <div key={event.id} onClick={() => { setSelectedEvent(event); setIsDescExpanded(false); setView('events_portal'); }} className="bg-slate-900 rounded-none overflow-hidden hover:shadow-2xl transition-all duration-500 flex flex-col group relative h-[320px] border border-gray-200 pointer-events-auto">
                         ...
                         <div className="absolute inset-0 z-0"><img src={event.image_url} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" alt={event.title} /></div>
                         ...
                         <div className="absolute bottom-0 left-0 right-0 p-6 opacity-0 translate-y-8 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500 z-30 pointer-events-none">
                            <p className="text-sm text-slate-300 line-clamp-3 mb-4 leading-relaxed">{event.description}</p>
                 ...
                 {dbEvents.length === 0 ? (
                   ...
                   <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                     {dbEvents.map((event) => {
                       ...
                       return (
                         <div key={event.id} onClick={() => { setSelectedEvent(event); setIsDescExpanded(false); }} className="bg-slate-900 rounded-none overflow-hidden hover:shadow-2xl transition-all duration-500 flex flex-col group relative h-[320px] border border-gray-200 pointer-events-auto">
                            <button onClick={(e) => toggleFavorite(e, event.id)} className={`absolute top-4 left-4 z-40 p-2.5 rounded-full transition-all duration-300 pointer-events-auto flex items-center justify-center border ${isFav ? 'bg-red-500 border-red-500 text-white shadow-md shadow-red-500/30 scale-100' : 'bg-transparent border-white/60 text-white hover:border-white hover:scale-105'}`}>
                               <svg className={`w-5 h-5 transition-transform ${isFav ? 'animate-pop' : ''}`} fill={isFav ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg>
                            </button>
                            <div className="absolute inset-0 z-0"><img src={event.image_url} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" alt={event.title} /></div>
                            ...
                            <div className="absolute bottom-0 left-0 right-0 p-5 sm:p-6 opacity-0 translate-y-8 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500 z-30 pointer-events-none">
                               <p className="text-xs sm:text-sm text-slate-300 line-clamp-3 mb-4 leading-relaxed">{event.description}</p>
             ...
             {bookingStep === 1 && selectedEvent && (
                <div className="w-full bg-white animate-in fade-in slide-in-from-bottom-4 relative">
                   <div className="h-64 sm:h-[450px] w-full bg-slate-900 relative rounded-none">
                      <img src={selectedEvent.image_url} className="w-full h-full object-cover opacity-60 rounded-none" alt={selectedEvent.title} />
                      <button onClick={() => setSelectedEvent(null)} className="absolute top-4 left-4 sm:top-6 sm:left-6 bg-white/20 backdrop-blur-md text-white px-3 py-1.5 sm:px-4 sm:py-2 rounded-none text-xs sm:text-sm font-bold hover:bg-white/40 transition-colors z-20 pointer-events-auto">‹ Zpět na přehled</button>
                      ...
                      <div className="flex-1 min-w-0">
                         <div className="w-full overflow-hidden">
                            <div className="flex justify-between items-center mb-4">
                                ...
                                <button onClick={(e) => toggleFavorite(e, selectedEvent.id)} className={`transition-all duration-300 pointer-events-auto flex items-center justify-center ${favoriteEvents.includes(selectedEvent.id) ? 'text-red-500' : 'text-slate-300 hover:text-slate-400 hover:scale-110'}`}>
                                   <svg className={`w-6 h-6 sm:w-8 sm:h-8 transition-transform ${favoriteEvents.includes(selectedEvent.id) ? 'animate-pop fill-current drop-shadow-sm' : 'fill-none'}`} stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg>
                                </button>
                            </div>
                            <div className="relative">
                               ...
                               <button onClick={() => setIsDescExpanded(!isDescExpanded)} className="text-red-600 text-xs sm:text-sm font-bold mt-2 hover:underline pointer-events-auto">{isDescExpanded ? 'Sbalit text' : 'Číst dále'}</button>
                            </div>
                         </div>
                      </div>
                      <div className="w-full md:w-80 shrink-0 self-start">
                         <div className="bg-white p-6 sm:p-8 rounded-none border border-gray-200 shadow-xl md:sticky md:top-24">
                            ...
                            <div className="mb-2 border-t border-gray-100 pt-6">
                              ...
                              {(() => {
                                const occupied = reservations.filter(r => r.event_id === selectedEvent.id && r.status !== 'cancelled').length;
                                ...
                                return (
                                  <>
                                    ...
                                    <button onClick={() => setBookingStep(2)} disabled={isFull} className={`w-full py-3 sm:py-4 mt-6 sm:mt-8 rounded-none text-xs sm:text-sm font-bold transition-all shadow-sm pointer-events-auto ${isFull ? 'bg-slate-100 text-slate-400' : 'bg-red-600 hover:bg-red-700 text-white active:scale-95 hover:shadow-lg'}`}>{isFull ? 'Vyprodáno' : 'Sem chci jít'}</button>
                                  </>
                                )
                              })()}
                            </div>
        ...
        {/* ===================== KROK 2 A 3 (Formulář a Dokončení) ===================== */}
        {view === 'events_portal' && bookingStep === 2 && selectedEvent && (
          <form onSubmit={handleClientSubmit} className="max-w-2xl mx-auto w-full bg-white p-6 sm:p-8 rounded-2xl shadow-sm border border-gray-100 space-y-6 sm:space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300 pointer-events-auto">
             <div style={{ display: 'none' }} aria-hidden="true"><input type="text" value={honeypot} onChange={e => setHoneypot(e.target.value)} tabIndex="-1" autoComplete="off" /></div>
             {!user && (
               <div className="bg-slate-50 border border-gray-200 rounded-xl p-3 sm:p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-0">
                 ...
                 <button type="button" onClick={() => { setIsLoginMode(true); setShowAuthModal(true); }} className="w-full sm:w-auto text-xs font-bold bg-white border border-gray-200 px-4 py-2 rounded-lg hover:border-slate-300 transition-colors">Přihlásit se</button>
               </div>
             )}
             ...
             <div>
               ...
               <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                 <div><label className="block text-[10px] sm:text-xs font-medium text-slate-500 mb-1 sm:mb-1.5">Jméno *</label><input type="text" required value={formData.firstName} onChange={e => setFormData({...formData, firstName: e.target.value})} className="w-full bg-slate-50 border border-gray-200 rounded-xl p-2.5 sm:p-3 text-xs sm:text-sm focus:border-red-500 outline-none" /></div>
                 <div><label className="block text-[10px] sm:text-xs font-medium text-slate-500 mb-1 sm:mb-1.5">Příjmení *</label><input type="text" required value={formData.lastName} onChange={e => setFormData({...formData, lastName: e.target.value})} className="w-full bg-slate-50 border border-gray-200 rounded-xl p-2.5 sm:p-3 text-xs sm:text-sm focus:border-red-500 outline-none" /></div>
                 <div><label className="block text-[10px] sm:text-xs font-medium text-slate-500 mb-1 sm:mb-1.5">E-mail *</label><input type="email" required value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} className="w-full bg-slate-50 border border-gray-200 rounded-xl p-2.5 sm:p-3 text-xs sm:text-sm focus:border-red-500 outline-none" /></div>
                 <div><label className="block text-[10px] sm:text-xs font-medium text-slate-500 mb-1 sm:mb-1.5">Telefon *</label><input type="tel" required value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} className="w-full bg-slate-50 border border-gray-200 rounded-xl p-2.5 sm:p-3 text-xs sm:text-sm focus:border-red-500 outline-none" /></div>
               </div>
             </div>
             
             <div className="pt-4 border-t border-gray-100">
                <div className="flex items-start gap-2 mb-4">
                   <input type="checkbox" id="gdprConsent" required checked={gdprConsent} onChange={e => setGdprConsent(e.target.checked)} className="mt-0.5 w-4 h-4 text-red-600 border-gray-300 rounded" />
                   <label htmlFor="gdprConsent" className="text-[10px] sm:text-xs text-slate-500 leading-snug">
                      Souhlasím se <span onClick={(e) => { e.preventDefault(); setShowGdprModal(true); }} className="text-red-600 hover:underline font-semibold">zpracováním osobních údajů</span> a s <span onClick={(e) => { e.preventDefault(); setShowVopModal(true); }} className="text-red-600 hover:underline font-semibold">Obchodními a storno podmínkami</span>. *
                   </label>
                </div>
        ...
        {view === 'events_portal' && bookingStep === 3 && lastCreatedRes && (
          <div className="max-w-md mx-auto w-full bg-white p-6 sm:p-8 rounded-2xl text-center space-y-4 sm:space-y-6 shadow-sm border border-gray-100 animate-in fade-in zoom-in-95 duration-500 pointer-events-auto">
             ...
             <button onClick={() => resetClientView()} className="w-full bg-slate-900 hover:bg-slate-800 text-white font-medium py-3 sm:py-3.5 text-xs sm:text-sm transition-all shadow-sm rounded-none">Hotovo, vrátit se na začátek</button>
          </div>
        )}
        ...
        {view === 'client_dashboard' && (
          <div className="max-w-4xl mx-auto w-full animate-in fade-in space-y-6 sm:space-y-8 pt-4 pb-12 pointer-events-auto">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
              <h2 className="text-2xl sm:text-3xl font-bold text-slate-900">Moje rezervace a vstupenky</h2>
              <button onClick={() => setView('events_portal')} className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold rounded-lg text-sm transition-colors">
                ‹ Zpět na akce
              </button>
            </div>
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
               {myReservations.length === 0 ? (
                  <div className="p-8 sm:p-12 text-center text-slate-500">Zatím nemáte žádné rezervace.</div>
               ) : (
                  <div className="divide-y divide-gray-50">
                     {myReservations.map(res => {
                        const eventObj = res.event_id ? dbEvents.find(e => e.id === res.event_id) : null;
                          ...
                          <div key={res.id} className="p-4 sm:p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center hover:bg-slate-50 transition-colors gap-4 sm:gap-0">
                             <div className="flex-1 w-full">
                                <div className="flex items-center gap-3 mb-1">
                                   <span className={`px-2 py-0.5 rounded text-[10px] font-bold tracking-wider uppercase ${res.event_id ? 'bg-purple-100 text-purple-700' : 'bg-slate-100 text-slate-600'}`}>{res.event_id ? 'Vstupenka' : 'Rezervace'}</span>
                                   <span className="font-bold text-slate-900 text-sm sm:text-base">{formatDateCzech(res.date)}</span>
                                </div>
                                <div className="text-xs sm:text-sm text-slate-500">{res.notes || `${res.start_hour}:00 - ${res.end_hour}:00`}</div>
                             </div>
                             <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-6 w-full sm:w-auto">
                                <div className="text-left sm:text-right">
                                   ...
                                   <div className={`text-[11px] sm:text-xs font-semibold mt-1 ${res.status === 'paid' ? 'text-green-600' : res.status === 'cancelled' ? 'text-red-500' : 'text-amber-500'}`}>{res.status === 'paid' ? 'Zaplaceno' : res.status === 'cancelled' ? 'Zrušeno' : 'Čeká'}</div>
                                </div>
                                {res.status === 'paid' && needsTicket && (
                                   <button onClick={() => handleShowTicket(res)} className="w-full sm:w-auto px-4 py-2 sm:py-2.5 bg-slate-900 text-white text-[10px] sm:text-xs font-bold uppercase tracking-widest rounded-lg hover:bg-slate-800 transition-colors flex items-center justify-center gap-2">
                                      <span className="text-base sm:text-lg">📷</span> Vstupenka
                                   </button>
                                )}
                             </div>
                          </div>
               ...
               <div className="mt-8 pt-8 border-t border-gray-100">
                 ...
                 <form onSubmit={handleProfilePasswordChange} className="space-y-4">
                   <div className="max-w-sm">
                     ...
                     <button type="submit" disabled={savingProfile} className="w-full sm:w-auto bg-slate-900 hover:bg-slate-800 text-white text-sm font-semibold rounded-xl transition-all shadow-sm h-12 px-8">
      ...
      {/* --- GLOBÁLNÍ MODALY --- */}
      <GlobalModals showCookieBanner={showCookieBanner} handleAcceptCookies={handleAcceptCookies} showGdprModal={showGdprModal} setShowGdprModal={setShowGdprModal} showVopModal={showVopModal} setShowVopModal={setShowVopModal} />
      <AuthModals showAuthModal={showAuthModal} setShowAuthModal={setShowAuthModal} isLoginMode={isLoginMode} setIsLoginMode={setIsLoginMode} isForgotPasswordMode={isForgotPasswordMode} setIsForgotPasswordMode={setIsForgotPasswordMode} resetEmailSent={resetEmailSent} setResetEmailSent={setResetEmailSent} authEmail={authEmail} setAuthEmail={setAuthEmail} authPassword={authPassword} setAuthPassword={setAuthPassword} authLoading={authLoading} handleAuthSubmit={handleAuthSubmit} gdprConsent={gdprConsent} setGdprConsent={setGdprConsent} setShowGdprModal={setShowGdprModal} setShowVopModal={setShowVopModal} showRecoveryModal={showRecoveryModal} setShowRecoveryModal={setShowRecoveryModal} newRecoveryPassword={newRecoveryPassword} setNewRecoveryPassword={setNewRecoveryPassword} handleRecoverySubmit={handleRecoverySubmit} />

      {/* KLIENTSKÝ TICKET (QR) MODAL */}
      {selectedTicket && (
        <div className="fixed inset-0 z-[9999] bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-4 pointer-events-auto" onClick={() => setSelectedTicket(null)}>
            <div className="bg-white p-6 sm:p-8 rounded-none max-w-sm w-full text-center shadow-2xl animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
                ...
                <div className="bg-slate-50 p-4 border border-gray-100 mb-6">{ticketQr ? <img src={ticketQr} alt="QR Ticket" className="mx-auto w-48 h-48 sm:w-64 sm:h-64 mix-blend-multiply" /> : <div className="w-48 h-48 sm:w-64 sm:h-64 bg-gray-200 animate-pulse mx-auto" />}</div>
                <p className="text-[10px] text-slate-400 font-mono break-all bg-slate-50 p-2 border border-gray-100">ID: {selectedTicket.id}</p>
                <button onClick={() => setSelectedTicket(null)} className="mt-8 w-full py-4 bg-slate-900 text-white text-sm font-bold uppercase tracking-widest transition-colors hover:bg-slate-800">Zavřít</button>
            </div>
        </div>
      )}
      
      {/* ZPRACOVÁNÍ REZERVACE MODAL */}
      {isSubmitting && (
        <div className="fixed inset-0 z-[9999] bg-slate-900/40 backdrop-blur-sm flex flex-col items-center justify-center p-4 pointer-events-auto">
           <div className="bg-white p-10 rounded-3xl shadow-2xl flex flex-col items-center max-w-sm w-full animate-in zoom-in-95">
              ...
              <h3 className="font-bold text-lg text-slate-900 mb-1">Zpracovávám...</h3>
